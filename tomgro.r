...
#' c("E" = 0.717, "Q10" = 1.4, "rm" = 0.016, "D" = 2.593, "K" = 0.58, "m" = 0.1, "Qe" = 0.0645, "tau" = 0.063)
...
biomass_growth_rate <- function(params =  c("E" = 0.717, "Q10" = 1.4, "rm" = 0.016,
    ...
    ## Table 3, Jones 1991
    pgred <- ifelse(T > 0 & T <= 12, 1.0 / 12.0 * T,
                    ifelse(T > 12 & T < 35, 1.0, 0))
...
mature_fruit_accumulation <- function(n = 3, T = 15, nodes_pp = 22, kappa = 5, Wf = 0 , Wm = 0){
    ## overall rate of development (per day) of fruit under current T
    ## Table 3, Jones 1991
    rf <-  ifelse(T > 8 | T <= 28, 0.0017 * T - 0.0147, ifelse(T > 28 & T < 35, 0.032, 0))
...
grow <- function(T, PPFD, manure_effect, N = 6, LAI = 0.006, tday){
    ...
    for(j in 1:length(T)){
        ...
        bgr <- bgr * (1 + manure_effect)
        dWfdt <- fruit_dry_matter_rate(T = T[j], bgr = bgr, n = N, tday = tday)
