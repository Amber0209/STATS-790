...
```{r}
# ---- Hard-code CRD efficiency results ----
df <- tribble(
  ~Criterion, ~Design,    ~Mean,   ~Min,    ~Max,
  "D-optimality efficiency",        "Cherry",   50.30, 17.76, 111.32,
  "D-optimality efficiency",        "Heirloom", 63.26, 20.76, 112.00,
  "D-optimality efficiency",        "Mixed",    49.97, 19.75,  83.58,
  "A-optimality efficiency",        "Cherry",   66.37, 17.01,  87.70,
  "A-optimality efficiency",        "Heirloom", 62.54, 20.54,  93.37,
  "A-optimality efficiency",        "Mixed",    44.70, 28.70,  81.80,
  "G-optimality efficiency",        "Cherry",   79.19, 66.47,  88.44,
  "G-optimality efficiency",        "Heirloom", 78.83, 66.01, 101.05,
  "G-optimality efficiency",        "Mixed",    69.37, 55.66,  93.11
)

ggplot(df, aes(x = Design, y = Mean)) +
  geom_pointrange(aes(ymin = Min, ymax = Max), 
                  fatten = 2, size = 0.5) +
  facet_wrap(~ Criterion, nrow = 1, scales = "free_y") +
  labs(
    title = "CRD Efficiency Relative to Gold Standard",
    y = "Efficiency", x = NULL
  ) +
...
generate_rcbd_Tempblock  <- function(env_data, n_designs = 100, formula,
...
rcbd_result <- generate_rcbd_Tempblock(env_data, n_designs = 100, formula = best_formula, n_blocks = 2)
...
rcbd_cherry <- generate_rcbd_Tempblock(
  env_data, 
  n_designs = 50, 
  formula = formula_cherry, 
...
rcbd_heirloom <- generate_rcbd_Tempblock(
  env_data, 
  n_designs = 50, 
  formula = formula_heirloom,  
...
```{r}
# ---- Hard-code RCBD temperature block efficiency results ----
df_rcbd <- tribble(
  ~Criterion, ~Design,          ~Mean,   ~Min,   ~Max,
  "D-optimality efficiency",        "Cherry",    30.95,   28.98,  32.92,
  "D-optimality efficiency",        "Heirloom",  33.21,   28.11,  37.00,
  "D-optimality efficiency",        "Mixed",     36.66,   21.79,  54.50,
  "A-optimality efficiency",        "Cherry",    41.83,   36.69,  46.97,
  "A-optimality efficiency",        "Heirloom",  38.12,   31.09,  45.51,
  "A-optimality efficiency",        "Mixed",     27.57,   18.65,  54.72,
  "G-optimality efficiency",        "Cherry",    66.05,   65.52,  66.58,
  "G-optimality efficiency",        "Heirloom",  65.18,   64.39,  74.94,
  "G-optimality efficiency",        "Mixed",     67.94,   64.58,  74.70
)

# ---- Plot: point-range (mean + min/max) ----
p <- ggplot(df_rcbd, aes(x = Design, y = Mean)) +
  geom_pointrange(aes(ymin = Min, ymax = Max),
                  fatten = 2, size = 0.5) +
  facet_wrap(~ Criterion, nrow = 1, scales = "free_y") +
  labs(
    title = "RCBD (Temperatur Blocks) Efficiency Relative to Gold Standard",
    x = NULL,
    y = "Efficiency"
  ) +
  theme_minimal(base_size = 13) +
...
rcbd_light <- generate_rcbd_lightblock(env_data, n_designs = 100, formula = best_formula, n_blocks = 3)
...
# ---- Hard-code PPFD-based RCBD efficiency results ----
df_ppfd <- tribble(
  ~Criterion, ~Design,          ~Mean,   ~Min,   ~Max,
  "D-optimality efficiency",        "Cherry",    48.51,   37.55,  63.73,
  "D-optimality efficiency",        "Heirloom",  49.94,   36.69,  76.73,
  "D-optimality efficiency",        "Mixed",     38.75,   24.26,  54.38,
  "A-optimality efficiency",        "Cherry",    51.43,   41.89,  63.88,
  "A-optimality efficiency",        "Heirloom",  55.78,   43.44,  68.02,
  "A-optimality efficiency",        "Mixed",     37.18,   18.50,  57.95,
  "G-optimality efficiency",        "Cherry",    76.39,   68.09,  87.98,
  "G-optimality efficiency",        "Heirloom",  85.24,   76.74,  99.12,
  "G-optimality efficiency",        "Mixed",     69.28,   68.29,  72.53
)

# ---- Plot: point-range (mean + min/max) ----
p_ppfd <- ggplot(df_ppfd, aes(x = Design, y = Mean)) +
  geom_pointrange(aes(ymin = Min, ymax = Max),
                  fatten = 2, size = 0.5) +
  facet_wrap(~ Criterion, nrow = 1, scales = "free_y") +
  labs(
    title = "RCBD (PPFD Blocks) Efficiency Relative to Gold Standard",
    x = NULL,
    y = "Efficiency"
  ) +
  theme_minimal(base_size = 13) +
