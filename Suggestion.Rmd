...
generate_DA_design_feedback <- function(opt_result, threshold = 1.3) {
  ...
  if (length(flagged_interactions) > 0) {
    interaction_suggestions <- make_interaction_suggestion(flagged_interactions)
  }
...
generate_G_design_feedback <- function(opt_result, student_df, env_data,
                                       threshold = 1.3, k_isolated = 2, top_n = 3) {
...
```{r}
generate_design_feedback(result_example, all_designs[["cherry-only"]][[12]], env_data)
```
